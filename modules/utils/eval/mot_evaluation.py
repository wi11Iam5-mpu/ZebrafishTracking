...
def calcMetrics(acc, task):
    ...
    acc.mot_events.to_csv(str(Path(args["outputPath"]) / "acc_mot_events.csv"))
...
def MOT_Evaluation(args):
    ...
    if useMOTFormat:
        gt_df = pd.read_csv(gt_csv, sep=",", header=None, usecols=[0, 1, 2, 3, 4],
                            names=['frame', 'id', '3d_x', '3d_y', '3d_z'])
    else:
    ...
    if task == "3D":
        ...
        if bboxCenter:
            ...
            if "cam1_aa_tl_x" in det_df.columns:
                det_df = det_df[
                    (det_df["cam1_aa_tl_x"] > 0) & (det_df["cam1_aa_w"] > 0) & (det_df["cam1_aa_tl_y"] > 0) & (
                            det_df["cam1_aa_h"] > 0)]
                det_df["cam1_x_bb"] = det_df["cam1_aa_tl_x"] + det_df["cam1_aa_w"] // 2
                det_df["cam1_y_bb"] = det_df["cam1_aa_tl_y"] + det_df["cam1_aa_h"] // 2
            else:
                det_df = det_df[
                    (det_df["aa_tl_x"] > 0) & (det_df["aa_w"] > 0) & (det_df["aa_tl_y"] > 0) & (det_df["aa_h"] > 0)]
        ...
        if bboxCenter:
            ...
            if "cam2_aa_tl_x" in det_df.columns:
                det_df = det_df[
                    (det_df["cam2_aa_tl_x"] > 0) & (det_df["cam2_aa_w"] > 0) & (det_df["cam2_aa_tl_y"] > 0) & (
                            det_df["cam2_aa_h"] > 0)]
        ...
        if "cam1_aa_tl_x" in det_df.columns:
            det_df = det_df[(det_df["cam1_aa_tl_x"] > 0) & (det_df["cam1_aa_tl_y"] > 0) & (det_df["cam1_aa_w"] > 0) & (
                    det_df["cam1_aa_h"] > 0)]
            ...
            det_df = det_df[
                (det_df["aa_tl_x"] > 0) & (det_df["aa_tl_y"] > 0) & (det_df["aa_w"] > 0) & (det_df["aa_h"] > 0)]
        ...
        if "cam2_aa_tl_x" in det_df.columns:
            det_df = det_df[(det_df["cam2_aa_tl_x"] > 0) & (det_df["cam2_aa_tl_y"] > 0) & (det_df["cam2_aa_w"] > 0) & (
                    det_df["cam2_aa_h"] > 0)]
...
if __name__ == "__main__":
    ...
    ap.add_argument("-task", "--task", type=str,
                    help="What kind of tracking to investigate [3D, cam1, cam2, cam1_bbox, cam2_bbox]")
    ...
    ap.add_argument("-useMOTFormat", "--useMOTFormat", action="store_true", default=True,
                    help="Use the MOT Challenge ground truth format")
    ap.add_argument("-thresh", "--thresh", type=float, default=0.5, help="Distance threshold")
    ...
    ap.add_argument("-sequences", "--sequences", nargs="*", type=str, default=[],
                    help="")

    ap.add_argument("-detail", "--detail", action="store_true", default=True,
                    help="Use the MOT Challenge ground truth format")

    global args, log_display
    ...
    target = "tracks_3d_interpolated"  # tracks_3d_interpolated tracklets_3d tracks_3d
    # target = "tracks_3d"

    if len(args["sequences"]) == 0:
        log_display = True
        sequences = ["ZebraFish-02", "ZebraFish-04"]
        # sequences = ["ZebraFish-01", "ZebraFish-02", "ZebraFish-03", "ZebraFish-04"]
        # sequences = ["ZebraFish-04"]
    else:
