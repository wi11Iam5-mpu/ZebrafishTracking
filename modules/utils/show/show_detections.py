...
detections = 'gt_h'  # 'yolo4_h_zy'
# detections = 'ddetr_h'  # 'yolo4_h_zy'
...
def main(seq_index='01'):
    ...
    # data_pack[0] = data_pack[0][['frame', 'aa_tl_x', 'aa_tl_y']]
    # data_pack[1] = data_pack[1][['frame', 'aa_tl_x', 'aa_tl_y']]
    data_pack[0].columns = ['frame', 'cx', 'cy', 'conf']
    data_pack[1].columns = ['frame', 'cx', 'cy', 'conf']
    selected = ['frame', 'cx', 'cy']
    data_pack_selected = [data[selected].values for i, data in enumerate(data_pack)]

    readers = [cv2.VideoCapture(str(p.resolve())) for p in raw_video_path]
        ...
        for n_frame in range(1, frame_count + 1):
            ...
            if n_frame in bbox[0].keys():
                for i, bb in enumerate(bbox[0][n_frame]):
                    cv2.rectangle(frame1, pt1=(int(bb[0]) - 13, int(bb[1]) - 13),
                                  pt2=(int(bb[0]) + 13,
                                       int(bb[1]) + 13),
            ...
            if n_frame in bbox[1].keys():
                for i, bb in enumerate(bbox[1][n_frame]):
                    cv2.rectangle(frame2, pt1=(int(bb[0]) - 25, int(bb[1]) - 25),
                                  pt2=(int(bb[0]) + 25,
                                       int(bb[1]) + 25),
                                  color=(225, 225, 225), thickness=10)
